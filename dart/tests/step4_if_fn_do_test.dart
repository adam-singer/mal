...
// ;; -----------------------------------------------------
  ...
  expectResults(testEval, '(def! gen-plus5 (fn* () (fn* (b) (+ 5 b))))', '', isFailing: true); // ignore this test output
  expectResults(testEval, '(def! plus5 (gen-plus5))', '', isFailing: true); // ignore this test output
  expectResults(testEval, '(plus5 7)', '12');
  expectResults(testEval, '(def! gen-plusX (fn* (x) (fn* (b) (+ x b))))','', isFailing: true); // ignore this test output
  expectResults(testEval, '(def! plus7 (gen-plusX 7))','', isFailing: true); // ignore this test output
  ...
  expectResults(testEval, '(def! sumdown (fn* (N) (if (> N 0) (+ N (sumdown  (- N 1))) 0)))', '', isFailing: true); // ignore this test output
  ...
  expectResults(testEval, '(def! fib (fn* (N) (if (= N 0) 1 (if (= N 1) 1 (+ (fib (- N 1)) (fib (- N 2)))))))', '', isFailing: true); // ignore this test output
